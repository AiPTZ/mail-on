...
export default async function AdminWorkspacePage({ params }: { params: { id: string } }) {
  const db = readDb();
  const workspace = db.workspaces.find((w) => w.id === params.id);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Peca ao cliente para publicar estes registros. Disparo so depois de verificado."
        actions={
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <section className="panel p-6 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-serif text-2xl">Registros DNS</h2>
            {domain ? (
          ...
          <p className="mb-4 text-sm text-cream/50">
            Cole SPF (TXT), DKIM (`pic._domainkey`), DMARC (`_dmarc`), tracking (CNAME `email.`) e MX se aparecer.
          </p>
          {!domain ? (
        ...
        <aside className="space-y-4">
          ...
          <div className="panel p-5">
            ...
            <p className="mt-2 text-sm">{user?.email || "sem usuario workspace"}</p>
          </div>
          <div className="panel p-5">
            <p className="label">Base</p>
            <p className="mt-2 text-sm">
              {lists} listas · {contacts} contatos
            </p>
          </div>
          <div className="panel p-5">
